...
function SearchInsightsTopTermsCard({
  ...
  const loadTermVideos = async (searchTerm: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/discovery/video/search-insights/videos?${params.toString()}`)
      if (!response.ok) {
        throw new Error(`Failed to load videos (${response.status})`)
      }
      const payload = await response.json()
      const mapped = (Array.isArray(payload?.items) ? payload.items : []).map((item: Record<string, unknown>) => ({
