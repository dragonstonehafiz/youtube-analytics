    ...
    async function load() {
      try {
        ...
        if (dataSourceLevel === 'channel') {
          outliersUrl = `http://localhost:8000/analytics/channel/outliers?start_date=${start}&end_date=${end}&metric=${metric}&granularity=${granularity}`
        } else {
          const videoIdsParam = videoIds.length > 0 ? `&video_ids=${videoIds.join(',')}` : ''
          const contentTypeParam = contentType ? `&content_type=${contentType}` : ''
          outliersUrl = `http://localhost:8000/analytics/video/outliers?start_date=${start}&end_date=${end}&metric=${metric}&granularity=${granularity}${videoIdsParam}${contentTypeParam}`
        }
        ...
        const built: SpikeRegion[] = await Promise.all(
          regions.map(async (region) => {
            ...
            const contributorsRes = await fetch(
              `http://localhost:8000/analytics/video/top-contributors?start_date=${region.start_date}&end_date=${region.end_date}&metric=${metric}${videoIdsParam}`,
