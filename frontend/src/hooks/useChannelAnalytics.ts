    ...
    async function load() {
      ...
      try {
        const buildUrl = (start: string, end: string) =>
          contentType === 'all'
            ? `http://localhost:8000/analytics/channel-analytics?start_date=${start}&end_date=${end}`
            : `http://localhost:8000/analytics/video-analytics/summary?start_date=${start}&end_date=${end}&content_type=${contentType}`
